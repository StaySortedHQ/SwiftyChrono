...
extension NSTextCheckingResult {
    ...
    func string(from text: String, atRangeIndex index: Int) -> String {
        ...
        text.subString(with: range(at: index))
...
extension NSRegularExpression {
    ...
    static func isMatch(forPattern pattern: String, in text: String) -> Bool {
        ...
        (try? NSRegularExpression(pattern: pattern, options: .caseInsensitive))?.firstMatch(in: text, range: NSRange(location: 0, length: text.count)) != nil
