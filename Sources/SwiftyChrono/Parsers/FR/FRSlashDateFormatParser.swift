...
public class FRSlashDateFormatParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: OptionValue]) -> ParsedResult? {
        ...
        if (match.isNotEmpty(atRangeIndex: openingGroup) && match.string(from: text, atRangeIndex: openingGroup) == "/") ||
        ...
        let openGroup = match.isNotEmpty(atRangeIndex: openingGroup) ? match.string(from: text, atRangeIndex: openingGroup) : ""
        ...
        let index = match.range(at: 0).location + match.range(at: openingGroup).length
